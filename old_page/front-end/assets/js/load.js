(function (global, undefined) {
  ...
  // Funkcija brez delovanja
  function noop() {}

  // Iterirajte po strukturi, ki je podobna polju
  ...
  // Izvleče ime datoteke iz naslova URL
  ...
  // Normalizirati opredelitev vira
  ...
  function preload(resource) {
    if (resource.state === undefined) {
      ...
      resource.onpreload = [];
      loadResource({ url: resource.url, type: "cache" }, () =>
        preloadDone(resource)
      );
    }
  }

  // Logika zaporedja nalaganja
  function load() {
    ...
    if (rest.length) {
      ...
      loadResourceWithCallback(
        ...
        isFunction(rest[0]) ? rest[0] : () => loader.load.apply(null, rest)
      );
    } else {
  ...
  function loadResourceWithCallback(resource, callback = noop) {
    ...
    if (resource.state === STATE.PRELOADING) {
      resource.onpreload.push(() =>
        loadResourceWithCallback(resource, callback)
      );
      return;
    }
  ...
  function onReady(resourceName, callback) {
    ...
    if (
      (res && res.state === STATE.LOADED) ||
      (resourceName === "ALL" && allLoaded() && domReady)
    ) {
      done(callback);
    } else {
  ...
  const loader = (global[headVar] =
    global[headVar] ||
    function () {
      loader.ready.apply(null, arguments);
    });

  // Nastavitev strategij nalaganja
  ...
  // Zaznavanje pripravljenosti DOM
