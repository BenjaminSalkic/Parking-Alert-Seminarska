...
class KParking {
  ...
  constructor(naslov, mesto) {
    ...
    this.redarInter = setInterval(() => {
      ...
      redarji.forEach((redar) => {
        ...
        let predvidevanaOddaljenost = izracunajRazdaljoVMetrih(
          this.#latituda,
          this.#longituda,
          redar.expected.latitude,
          redar.expected.longitude
        );
  ...
  // Metoda za vnos lokacije GPS (zaušnica - v praksi bi uporabili dejanski vmesnik API GPS)
  vnesiLokacijaGPS() {
    // Simulirane GPS koordinate
  ...
  // določi najbližje parkirno mesto z vstavljenimi koordinatami
  najdiNajblizjiParking() {
    let selPark = null;
