class ZMStudent {
      ...
      navigator.geolocation.getCurrentPosition(
        async (position) => {
          ...
          try {
            ...
            // Posodobitev DOMa, če je to del posodobitve uporabniškega vmesnika
  ...
  async vklopiGPS() {
    ...
    await this.gps.vklopiGPS();
    this.location = this.gps.getLokacija();
  }
  ...
  izklopiGPS() {
    ...
    // Izbriši lokacijo iz uporabniškega vmesnika
    const locationSpan = document.getElementById("last-location");
    if (locationSpan) {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (registerForm) {
    ...
    registerForm.addEventListener("submit", function (e) {
      ...
      // Počakaj nekaj časa in preusmerite na prijavno stran
      setTimeout(() => {
        window.location.href = "/front-end/prijava.html";
      }, 1500);
    });
...
function logout() {
  ...
  if (userData) {
    ...
    console.log("Ustvarjen primerek ZMStudent:", uporabnik);
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (loggedInUser) {
    ...
    document.getElementById("studentImage").src =
      user.image ||
      "https://coffective.com/wp-content/uploads/2018/06/default-featured-image.png.jpg"; // Default fallback slika
  }
});
...
function editField(fieldId, icon) {
  ...
  if (isReadOnly) {
    ...
    const loggedInUser = JSON.parse(localStorage.getItem("loggedInUser"));
    loggedInUser[fieldId] = updatedValue;
